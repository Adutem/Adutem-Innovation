...
const Navbar = ({ toggleNavBar, isNavbarOpen }: NavbarProps) => {
  ...
  // Holiday state
  const { activeHoliday } = useStateSelector((state) => state.Holiday);
  ...
  return (
    <>
      <Container
        className="fixed top-0 left-0 w-full bg-[transparent] maa-transition-[0.5s]"
        ref={containerRef as any}
      >
        {activeHoliday && (
          <div className="bg-[var(--base-color)] w-full h-16 flex items-center justify-center">
            <p className="neue-regular text-white text-xl md:text-2xl text-center w-full font-semibold">
              {activeHoliday.textContent}
            </p>
          </div>
        )}
        <div className="flex justify-between items-center px-4 md:px-6 w-full py-4">
          <Heading
